  ...
  const Skills = () => {
    ...
    return (
      <div id='skills' className="font-[font2] min-h-screen pt-40 lg:pt-50 px-5 lg:px-20 pb-25">
        ...
        <div className="mt-20 flex flex-col gap-5">
          {categories.map((category, index) => (
            <div key={category.title}>
              {/* Category heading */}
              <div
                className="heading relative lg:h-35 w-full overflow-hidden border-zinc-200 border rounded-xl p-8 pt-13 flex justify-between cursor-pointer z-8"
                onClick={() => toggleCategory(index)}
              >
                <div className="subheading h-full w-full bg-zinc-100 absolute top-0 right-0 rounded-xl opacity-0"></div>
                <h1 className="uppercase text-xl lg:text-4xl bg-gradient-to-r from-orange-700 to-orange-300 bg-clip-text text-transparent z-8">
                  {category.title}
                </h1>
                <img
                  src={arrow}
